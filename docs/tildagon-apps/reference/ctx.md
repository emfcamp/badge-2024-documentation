...
| `get_font_name(i)` | Get the name of a loaded font by index. Only one font is built into the firmware at index `0`: `"EMF Camp Font"`. Raises `ValueError` if no font exists at the given index. | `i`: The index of the font. | `font_name`: The name of the font. |
